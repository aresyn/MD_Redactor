...
Перед сохранением WPF получает Markdown из WebView2, запускает `EditTagValidator` и записывает файл только если нет ошибок уровня `Error`. При ошибках разметки статус становится `Ошибка разметки правок`, пользователю показывается список русских diagnostics со строкой и колонкой, исходный файл не меняется.
...
При закрытии окна или открытии другого файла с dirty-состоянием WPF показывает русское подтверждение с кнопками `Сохранить`, `Не сохранять`, `Отмена`. Dirty=false закрывается без подтверждения.
