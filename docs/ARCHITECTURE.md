...
Тема выбирается в верхней панели: `Системная`, `Светлая`, `Темная`. Настройка хранится в `%LOCALAPPDATA%\MDRedactor\settings.json`; это настройка приложения и не является хранилищем правок. WPF разрешает системную тему в фактическое значение `light` или `dark` и отправляет его в web/editor через `host.setTheme`.
